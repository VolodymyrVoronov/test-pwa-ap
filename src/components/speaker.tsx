    ...
    return (
      <AudioPlayerButton
        ...
        item={
          player.activeItem
            ? {
                id: tracksFiles[currentTrackIndex].id,
                src: tracksFiles[currentTrackIndex].url,
                data: { name: tracksFiles[currentTrackIndex].name },
              }
            : undefined
        }
        className={cn(
...
function SpeakerControls({
  ...
  useEffect(() => {
    const track = {
      id: tracksFiles[0].id,
      src: tracksFiles[0].url,
      data: { name: tracksFiles[0].name },
    };
  ...
  const playTrack = useCallback(
    (index: number) => {
      ...
      const track = {
        id: tracksFiles[index].id,
        src: tracksFiles[index].url,
        data: { name: tracksFiles[index].name },
      };
  ...
  const nextTrack = () => {
    const nextIndex = (currentTrackIndex + 1) % tracksFiles.length;
    playTrack(nextIndex);
  };

  const prevTrack = () => {
    const prevIndex =
      (currentTrackIndex - 1 + tracksFiles.length) % tracksFiles.length;
    playTrack(prevIndex);
  };
  ...
  const tracks = tracksFiles.map((t) => ({
    id: t.id,
    title: t.name,
